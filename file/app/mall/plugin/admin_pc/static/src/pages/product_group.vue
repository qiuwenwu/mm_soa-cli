						...
						<mm_card>
							...
							<div class="card_body">
								<mm_form class="bar_filter">
									...
									<mm_list col="3">
										<mm_item>
											<control_input v-model="query.keyword" title="关键词" desc="分组名称 / 分组标题 / 分组描述"
											 @blur="search()" />
										</mm_item>
										<mm_item>
								...
								<div class="bar_action">
									<h5><span>操作</span></h5>
									<div class="btns">
										<mm_btn class="btn_primary-x" url="./product_group_form?">添加</mm_btn>
								...
								<mm_table type="2">
									<thead class="table-sm">
										<tr>
											...
											<th>
												<control_reverse title="分组名称" v-model="query.orderby" field="name" :func="search"></control_reverse>
											</th>
											<th>
												<control_reverse title="分组标题" v-model="query.orderby" field="title" :func="search"></control_reverse>
											</th>
											<th>
												<control_reverse title="分组描述" v-model="query.orderby" field="description" :func="search"></control_reverse>
											</th>
											<th>
									...
									<tbody>
										...
										<tr v-for="(o, idx) in list" :key="idx" :class="{'active': select == idx}" @click="selected(idx)">
											...
											<td>
												<mm_btn class="btn_primary" :url="'./product_group_form?group_id=' + o[field]">修改</mm_btn>
...
<script>
	...
	export default {
		...
		data() {
			return {
				// 列表请求地址
				url_get_list: "/apis/mall/product_group",
				url_del: "/apis/mall/product_group?method=del&",
				url_set: "/apis/mall/product_group?method=set&",
				url_import: "/apis/mall/product_group?method=import&",
				url_export: "/apis/mall/product_group?method=export&",
				field: "group_id",
				query_set: {
					"group_id": ""
				},
