						...
						<mm_card>
							...
							<div class="card_body">
								<mm_form class="bar_filter">
									...
									<mm_list col="3">
										<mm_item>
											<control_input v-model="query.keyword" title="关键词" desc="标题 / 描述 / 关键词 / 标签"
											 @blur="search()" />
										</mm_item>
										<mm_item>
										...
										<mm_item>
											<control_select v-model="query.type_id" title="店铺分类" :options="$to_kv(list_shop_type, 'type_id', 'name')"
											 @change="search()" />
										</mm_item>
										<mm_item>
											<control_select v-model="query.channel_id" title="频道" :options="$to_kv(list_shop_channel, 'channel_id', 'name')"
											 @change="search()" />
										</mm_item>
										<mm_item>
										...
										<mm_item>
											<control_select v-model="query.user_id" title="店铺所属人" :options="$to_kv(list_account, 'user_id', 'nickname')"
											 @change="search()" />
										</mm_item>
										<mm_item>
								...
								<mm_table type="2">
									<thead class="table-sm">
										<tr>
											...
											<th>
												<control_reverse title="店铺分类" v-model="query.orderby" field="type_id" :func="search"></control_reverse>
											</th>
											<th>
											...
											<th>
												<control_reverse title="店铺所属人" v-model="query.orderby" field="user_id" :func="search"></control_reverse>
											</th>
											<th>
											...
											<th>
												<control_reverse title="标题" v-model="query.orderby" field="name" :func="search"></control_reverse>
											</th>
											<th>
									...
									<tbody>
										...
										<tr v-for="(o, idx) in list" :key="idx" :class="{'active': select == idx}" @click="selected(idx)">
											...
											<td>
												<span>{{ get_name(list_shop_type, o.type_id, 'type_id', 'name') }}</span>
											</td>
											<td>
												<input class="input_display" v-model.number="o.display" @blur="set(o)" min="0" max="1000" />
											</td>
											<td>
												<span>{{ get_name(list_shop_channel, o.channel_id, 'channel_id', 'name') }}</span>
											</td>
											<td>
			...
			<mm_card class="card">
				...
				<div class="card_body">
					<dl>
						...
						<dt>店铺分类</dt>
						<dd>
							<control_select v-model="form.type_id" :options="$to_kv(list_shop_type, 'type_id', 'name')" />
						</dd>
						<dt>频道</dt>
						<dd>
							<control_select v-model="form.channel_id" :options="$to_kv(list_shop_channel, 'channel_id', 'name')" />
						</dd>
						<dt>所属城市</dt>
						<dd>
	...
	export default {
		...
		data() {
			return {
				...
				url_import: "/apis/mall/shop?method=import&",
				url_export: "/apis/mall/shop?method=export&",
				field: "shop_id",
				query_set: {
					"shop_id": ""
				},
		...
		methods: {
			...
			get_shop_channel(query) {
				...
				this.$get('~/apis/mall/shop_channel?size=0', query, function(json) {
					if (json.result) {
						_this.list_shop_channel.clear();
						_this.list_shop_channel.addList(json.result.list)
					}
				});
			},
			/**
