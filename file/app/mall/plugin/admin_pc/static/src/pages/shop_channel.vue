							...
							<div class="card_body">
								<mm_form class="bar_filter">
									...
									<mm_list col="3">
										...
										<mm_item>
											<control_select v-model="query.father_id" title="上级" :options="$to_kv(list_shop_channel, 'channel_id', 'name')"
											 @change="search()" />
										</mm_item>
										<mm_item>
								...
								<div class="bar_action">
									<h5><span>操作</span></h5>
									<div class="btns">
										<mm_btn class="btn_primary-x" url="./shop_channel_form?">添加</mm_btn>
								...
								<mm_table type="3">
									<thead class="table-sm">
										<tr>
											...
											<th>
												<control_reverse title="频道类型" v-model="query.orderby" field="type" :func="search"></control_reverse>
											</th>
											<th>
									...
									<tbody>
										...
										<tr v-for="(o, idx) in list_new" :key="idx" :class="{'active': select == idx, sub: o[father_id], open: opens_has(o[field]), no_sub: !opens_has_sub(o[field]) }"
										 @click="selected(idx)">
											...
											<td>
												<span>{{ get_name(list_shop_channel, o.father_id, 'channel_id', 'name') }}</span>
											</td>
											<td>
											...
											<td>
												<mm_btn class="btn_primary" :url="'./shop_channel_form?channel_id=' + o[field]">修改</mm_btn>
			...
			<mm_card class="card">
				...
				<div class="card_body">
					<dl>
						...
						<dt>上级</dt>
						<dd>
							<control_select v-model="form.father_id" :options="$to_kv(list_shop_channel, 'channel_id', 'name')" />
						</dd>
						<dt>所属城市</dt>
						<dd>
...
<script>
	...
	export default {
		...
		data() {
			return {
				// 列表请求地址
				url_get_list: "/apis/mall/shop_channel",
				url_del: "/apis/mall/shop_channel?method=del&",
				url_set: "/apis/mall/shop_channel?method=set&",
				url_import: "/apis/mall/shop_channel?method=import&",
				url_export: "/apis/mall/shop_channel?method=export&",
				field: "channel_id",
				query_set: {
					"channel_id": ""
				},
